<div>
    <div class="form-group" wire:ignore>
        <div class="row d-flex justify-content-between ml-1 mr-1">
            ...
            <div>
                <button type="button" class="btn btn-danger btn-sm" wire:click="clearCombinations" title="Clear All Combinations">Clear</button>
            </div>
        </div>
    ...
    @foreach ($selectedOptions as $index => $value)
        <div class="row">
            ...
            <div class="col-md-8">
                <div class="form-group ">
                    <input type="text" class="form-control product-variante-inpute" placeholder="Enter product Variante"
                        wire:model.lazy="variante.{{ $value }}" wire:keydown.enter.prevent="add">
                </div>
            </div>
        </div>
    @endforeach
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        <input type="text" class="form-control" placeholder="Enter product price"
                            wire:model.lazy="subVariantePrice.{{ implode('-', $combination) }}"
                            wire:keydown.enter.prevent="subVariater">
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group">
                        <input type="number" min="0" class="form-control" placeholder="stock"
                            wire:model.lazy="subVarianteStock.{{ implode('-', $combination) }}">
                    </div>
                </div>
            </div>
        @endforeach
